  ...
  const rmf_traffic::agv::VehicleTraits& vehicle_traits)
{
  ...
  if (!graph_config)
  {
    std::cerr << "Failed to load graph file [" << graph_file << "]" 
        << std::endl;
  ...
  if (!levels)
  {
    std::cerr << "Graph file [" << graph_file 
        << "] is missing the [levels] key" << std::endl;
    return rmf_utils::nullopt;
  }

  if (!levels.IsMap())
  {
    std::cerr << "The [levels] key does not point to a map in graph file ["
        << graph_file << "]" << std::endl;
    return rmf_utils::nullopt;
  }
  ...
  for (const auto& level : levels)
  {
    ...
    for (const auto& vertex : vertices)
    {
      ...
      const auto& wp = info.graph.add_waypoint(map_name, location);
      ...
      if (name_option)
      {
        ...
        if (!name.empty())
        {
          ...
          if (!ins.second)
          {
            std::cerr << "Duplicated waypoint name [" << name << "] in graph ["
                << graph_file << "]" << std::endl;
            return rmf_utils::nullopt;
          }
      ...
      if (workcell_name_option)
      {
        ...
        info.workcell_names.insert(std::make_pair(wp.index(), workcell));
      }
    ...
    for (const auto& lane : lanes)
    {
      ...
      if (orientation_constraint_option)
      {
        ...
        {
          std::cerr << "Unrecognized orientation constraint label given to lane"
              << "[" << std::to_string(lane[0].as<std::size_t>()) << ", "
              << std::to_string(lane[1].as<std::size_t>()) << "]: ["
              << constraint_label << "] in graph [" << graph_file << "]" 
              << std::endl;
          return rmf_utils::nullopt;
        }
      }
      ...
      if (const YAML::Node mock_lift_option = options["demo_mock_floor_name"])
      {
        ...
        if (!lift_name_option)
        {
          std::cerr << "Missing [demo_mock_lift_name] parameter which is "
              << "required for mock lifts" << std::endl;
          return rmf_utils::nullopt;
        }
